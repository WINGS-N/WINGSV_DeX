	...
	for {
		...
		case "apps":
			...
			if !cmd.Whitelist {
				...
				// pick the tunnel IP as their source. Masquerade their mark-tagged egress so it
				// leaves the physical link with a valid source and replies return.
				if err := wg.SetBypassMasquerade(true, cmd.AppMark); err != nil {
